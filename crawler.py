...
def fetch_nejm():
    ...
    for entry in feed.entries[:5]:
        ...
        articles.append({'category': '最新研究','title': entry.title,'summary': clean + '...','source': 'NEJM'})
    return articles

def fetch_pubmed():
    base = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/'
    params = {'db':'pubmed','term':'("2026"[Date - Publication] : "3000"[Date - Publication]) AND ("N Engl J Med"[Jour] OR "Lancet"[Jour] OR "JAMA"[Jour])','retmax':5,'sort':'pub_date','retmode':'json'}
    try:
        ...
        for pid in ids:
            p = results.get(pid, {})
            articles.append({'category':'權威期刊','title': p.get('title','未知'),'summary': f"{p.get('source','')} ({p.get('pubdate','')})",'source':'PubMed'})
        return articles
...
def main():
    ...
    if not all_articles:
        all_articles.append({'category':'系統訊息','title':'今日無自動抓取之新聞','summary':'請稍後再試。','source':'永遠亭'})
